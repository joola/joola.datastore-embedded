...
EmbeddedProvider.prototype.openConnection = function (options, callback) {
  callback = callback || function () {
  };

  var self = this;

  //expected DSN format: embedded://[username:password@]host1[:port1][,host2[:port2],...[,hostN[:portN]]][/[database][?options]]
  //reference: http://docs.embedded.org/manual/reference/connection-string/

  //check if we have a cached connection
  if (self.dbs[options.dsn])
    return self.checkConnection(self.dbs[options.dsn], callback);

  this.logger.trace('Open Embedded connection @ ' + this.options.dsn);
  if (!self.client)
...
EmbeddedProvider.prototype.drop = function (collectionName, callback) {
  ...
  return self.db.dropCollection(cleanup(collectionName), function (err) {
    if (err.message.indexOf('ns not found') > -1) //allow this error
      return callback(null, {});
    return callback(err);
  });
};
